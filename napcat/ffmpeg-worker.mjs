    ...
    const [fileInfo, durationInfo] = await Promise.all([
      ...
      (async () => {
        const [fileType, fileSize] = await Promise.all([
          withTimeout(fileTypeFromFile(videoPath), 1e4, "获取文件类型").then((result) => {
            return result;
          }),
          (async () => {
        ...
        try {
          ...
          try {
            ...
            sendLog(`清理输出文件失败: ${error.message}`);
          }
          try {
            ffmpegInstance.fs.unlink(videoFileName);
          } catch (error) {
            sendLog(`清理视频文件失败: ${error.message}`);
          }
        }
      })(),
      // 任务2: 获取视频时长
      (async () => {
        ...
        try {
          ...
          try {
            ...
            sendLog(`清理输入文件失败: ${error.message}`);
          }
        }
      })()
    ]);
...
export { handleFFmpegTask as default, recvTask, sendLog };
